...
parser = argparse.ArgumentParser(description='Train Td-PN')
...
learning_rate = tf.train.exponential_decay(lr, global_step,
                                           step_size, gamma, staircase=False)
...
for ep in range(int(iters / 100), n_epochs):
    ...
    for epi in tqdm(range(n_episodes), desc='train epoc:{}'.format(ep)):
        support, s_labels, query, q_labels = loader_train.next_data(n_way, n_shot, n_query)
        support = support / 255.0
        query = query / 255.0

        _, summaries, step, ls, ac = sess.run([train_op, train_summary_op, global_step, ce_loss, acc],
                                              feed_dict={m.x: support, m.ys: s_labels, m.q: query, m.y: q_labels,
                                                         m.phase: 1})
    ...
    for epi in tqdm(range(n_episodes), desc='val epoc:{}'.format(ep)):
        ...
        support, s_labels, query, q_labels = loader_val.next_data(n_test_way, n_test_shot, n_test_query)
        support = support / 255.0
        query = query / 255.0
        summaries, vls, vac = sess.run([val_summary_op, ce_loss, acc],
                                       feed_dict={m.x: support, m.ys: s_labels, m.q: query, m.y: q_labels, m.phase: 0})
    ...
    print('epoch:{}, loss:{:.5f}, acc:{:.5f}, val, loss:{:.5f}, acc:{:.5f}, lr:{:.8f}'.format(ep, np.mean(loss_tr), np.mean(acc_tr), np.mean(loss_val), np.mean(acc_val), lr))
